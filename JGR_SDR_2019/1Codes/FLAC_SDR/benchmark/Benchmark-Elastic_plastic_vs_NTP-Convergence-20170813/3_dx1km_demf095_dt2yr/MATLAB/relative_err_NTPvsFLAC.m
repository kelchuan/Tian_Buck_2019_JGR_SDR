...
for i = n:n
%    for i = n:n
data_x = particles_1((i_start_particles:size_particle(1)),1,i);
data_y = w_flac((i_start_particles:size_particle(1)),2,i);
if i==n
...
% Initialization
%----------------------------------------------------------------
scale_factor_dV0 = 1 % this decrease the load due to resolution change
N_SDR = 30;    %total number of SDRs
Length = 800000;             %[m] total length 
N = 801;               % number of node points
...
[p,S] = polyfit(x(1:dN_sampling:length(x)),wn(1:dN_sampling:length(x)),npoly);
...
residual=sum((wn(1:dN_sampling:length(x))-Y(1:dN_sampling:length(x))).^2);
norm_polyfit = sqrt(residual)/length(wn(1:dN_sampling:length(x)));
%}
...
YY = polyval(p,data_x); % get the NTP YY corresponding to the data_x location
residual=sum((data_y(1:dN_sampling:length(data_x))-YY(1:dN_sampling:length(data_x))).^2);
%L2_norm = sqrt(residual)/length(data_x);
L2_norm = sqrt(residual/length(data_y(1:dN_sampling:length(data_x))));
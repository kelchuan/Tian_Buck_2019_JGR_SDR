...
SDR_1 = plot(data_x1,data_y1,'r-.', 'Linewidth', 2); hold on;
SDR_2 = plot(data_x2,data_y2,'y-.', 'Linewidth', 2); hold on;
SDR_3 = plot(data_x3,data_y3,'g-.', 'Linewidth', 2); hold on;
SDR_4 = plot(data_x4,data_y4,'m-.', 'Linewidth', 2); hold on;
SDR_5 = plot(data_x5,data_y5,'b-.', 'Linewidth', 2); hold on;
...
npoly = 36;   % degree of the fitting polynomial
...
plot(reso_x/1000, Yp2, 'r-.', 'Linewidth', 6)
S2.normr
hold on;
...
figure
npoly_log = 1;
reso_log = log10(reso(1:4));  
%ignore the fifth one because the fifth error is the error between polyfit
%of the highest resolution of 250m to the real data
% first four error are the point averaged difference between models to the
% highest resolution model of 250m
L1_norm_avg_log = log10(L1_norm_avg(1:4));
[p2_log,S2_log] = polyfit(reso_log,L1_norm_avg_log,npoly_log);
reso_x_log = linspace(min(reso_log)-0.5,max(reso_log)+0.5,100);
Yp2_log = polyval(p2_log,reso_x_log);
plot(reso_x_log, Yp2_log, 'r-.', 'Linewidth', 3)
S2.normr
hold on;

plot(reso_log, L1_norm_avg_log,'b.', 'Markersize', 98); hold on;
grid on
xlabel("log_{10}(grid size) [m]")
ylabel("log_{10}(point avg error) [m]")
set(gca,'Fontsize',26','Linewidth',3)
axis([2.6 3.6 0.5 2])

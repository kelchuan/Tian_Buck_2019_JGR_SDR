...
!100 continue
do 100 i=1,np
!   xp(i,:)=dble(x0)+dble(dxp)*dble(i-1)   !Tian20170801
   xp(i,:)=dble(x0)+dble(1001)+dble(dxp)*dble(i-1)   !500 means starting from the center of the dike with 1km width
!   xp(i,:)=dble(x0)+dble(npelem)*dble(dxp)*2*2+dble(dxp)*dble(i-1)   !(+npelem*dxp to shift off axis of 1 elem dx
...
!   zp(i,:) = -200.0d0 ! all zp start at mid poit of the surface element
   zp(i,:) = -0.1d0 ! all zp start at the surface element Tian 20170720
!   zp(i,:) = -0.01d0 ! all zp start at the surface element Tian 20170801
!      write(*,*) 'zp  ', i, '=', zp(i,1)
!   write(*,*) 'xp(', i,') = ', xp(i,1)
   !   write(*,*) 'zp(', i,') = ', zp(i,1)
   !   write(*,*) xp(i,1), zp(i,1)

...
!        endif
        ...
        if (irh.eq.3 .or. irh.eq.12) then 
           if (ny_inject.eq.0.and.i.eq.1) visn(j,i) = 1.e+17 !Tian170713 remake dike viscosity to be low everytime
           !            if( mod(nloop,ifreq_visc).eq.0 .OR. ireset.eq.1 ) visn(j,i) = Eff_visc(j,i)
           !Tian170713 the above line is commented for constant visc
...
!            if (ny_inject.gt.0.and.i.eq.iinj) aps (j,i) = 0.
            if (ny_inject.gt.0.and.i.lt.iinj+3) aps (j,i) = 0. !Tian wider zero plastic strain zero zone

...
function Eff_dens( j, i)
  ...
  enddo
  press = press / 12
  !Tian 20170813 hardwired in order to have no density change due to mixture of phases
  kkk = 0 ! Tian just for getting the if statement for no phase mixture effects on density
...
!!$      Eff_dens = dens
      ...
      do k = 1, nphase
          ...
          write(*,*) "Eff_dens (before) = " , Eff_dens !Tian for debuging density change 170813 
          Eff_dens = Eff_dens + ratio*dens
!          write(*,*) "Eff_dens (after) = " , Eff_dens !Tian for debuging density change 170813           
!          write(*,*) "ratio=", ratio, "dens= ", dens!Tian for debuging density change 170813           
      enddo
  endif
  return

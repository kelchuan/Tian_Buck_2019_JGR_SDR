  ...
  do i = 1,nx-1  
      do j = 1,nz-1
          ...
          if (irh.ne.11) then
             ...
             !Tian 20170713 commented for two layered viscosity struc
             !          else
             !Tian 20170713 commented for two layered viscosity struc
              ...
              ! for two layer viscosity case Tian20170713
              if(ivis_shape.eq.3) then
                 if (yc .gt. geoth) visn(j,i)=v_max
                 if (yc .le. geoth) visn(j,i)=v_min
              endif
              ! for broken plate set the axis with low viscosity
              if(i.eq.1) then
                 visn(j,i) = 1.e+16
              endif
              ! min and max cutoffs

...
xfit = [particles_1(1,1,i),particles_2(1,1,i),particles_3(1,1,i),...
    ...
    particles_13(1,1,i),particles_14(1,1,i),particles_15(1,1,i),particles_16(1,1,i),0];
yfit = [particles_1(1,2,i),particles_2(1,2,i),particles_3(1,2,i),...
    ...
    particles_13(1,2,i),particles_14(1,2,i),particles_15(1,2,i),particles_16(1,2,i),0];   
npoint = size(xfit);
hold on;
plot(xfit,yfit,'k+','markersize',18); hold on;
nDegree = 6
pSDR = polyfit(xfit(6:npoint(2)),yfit(6:npoint(2)),nDegree);
NN = 1000; 
Xend = particles_6(1,1,i);
xfit_array = linspace(0,Xend,NN);
yfit_array = polyval(pSDR,xfit_array);
plot(xfit_array,yfit_array,'b-.','linewidth',3); hold on;
[pks ploc] = findpeaks(-yfit_array);
%Plot Xf
%Xf = -pSDR(2)/(2*pSDR(1))
Xf = ploc/NN * Xend
...
eval(sprintf("Kt = (particles_%d(2,2,n)-particles_%d(1,2,n)) / (particles_%d(2,1,n)-particles_%d(1,1,n));",ixfit,ixfit,ixfit,ixfit))
phi = atand(Kt)   % verified with on screen measurement, very good
%W0 = abs(particles_6(1,2,n))
eval(sprintf("W0 = abs(particles_%d(1,2,n))",ixfit));
gamma = W0 / tand(phi);

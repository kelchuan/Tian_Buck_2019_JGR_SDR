...
dw = dw_0 * exp(-x/alpha).*cos(x/alpha);
%figure
%plot(x,dw,'r+')
...
figure
plot(x/1000, -dw,'ro'); hold on;
plot(x/1000, wn,'k.');
title("Comparison between NTP and ATP in dw",'Fontsize',26')
xlabel('Distance from the axis [km]','Fontsize',26');
ylabel('Depth [m]','Fontsize',26');
set(gca,'Fontsize',26','Linewidth',3)
grid on;
...
figure
plot(x/alpha, -dw,'ro'); hold on;
plot(x/alpha, wn,'k.');
grid on;
xlabel('Distance from the axis normalized with alpha[\alpha]','Fontsize',26');
ylabel('Depth [m]','Fontsize',26');
title("Comparison between NTP and ATP in dw",'Fontsize',26')
set(gca,'Fontsize',26','Linewidth',3)
set(gcf,'position',[x0,y0,width,height])
...
dif_AN_eq3 = wt + w;
plot(x(3:N)/1000, (wt(3:N)-dif_AN_eq3(3:N)),'r--', 'Linewidth', 3.5);hold on;
%legend();
...
for i = 0:1:30
    ...
    plot(x(floor(x_0/Length*N)+1:N)/1000, ws(floor(x_0/Length*N)+1:N)...
        -dif_AN_eq3(floor(x_0/Length*N)+1:N),'r--', 'Linewidth', 3);hold on;
...
ylabel('Depth [m]','Fontsize',26');
set(gca,'Fontsize',26','Linewidth',3)
title({"Comparison between ATP (blue) and NTP corrected with",...
    " ATP NTP difference in dike-lava interface eq3 (red)"})
set(gcf,'position',[x0,y0,width,height])

%difference between analytic and numerical thin plate in terms of eq3
%(Boundary between dike and SDRs)
w_0 = Hd * (delta_rho_d / delta_rho_c);
w = w_0 * exp(- x / alpha) .* (sin(x/alpha) - cos(x/alpha)) + w_0; %(eq.3)
dif_AN_eq3 = wt + w;
figure
%plot(x, dif_AN_eq3, 'b+')
plot(x/alpha, dif_AN_eq3, 'b+')
grid on
set(gca,'Fontsize',26','Linewidth',3)
title({"ATP and NTP difference in dike-lava interface eq3"})
xlabel('Distance from the axis [km]','Fontsize',26');
ylabel('Depth difference [m]','Fontsize',26');
